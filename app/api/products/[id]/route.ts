...
export async function PUT(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    db.prepare(`
      ...
      WHERE id = ?
    `).run(name, description, price, original_price || null, category_id, brand, stock, image_url, JSON.stringify(images || []), featured ? 1 : 0, weight || null, flavor || null, servings || null, active !== false ? 1 : 0, id);
